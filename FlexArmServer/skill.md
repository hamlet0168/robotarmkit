...
- **独立机械臂标定程序**: 'RobotArmCalibration.exe'
  下载地址：[官方仓库 Release 页面](https://github.com/hamlet0168/flexarm/releases)  
  最新版本：v2.0.0  
  文件大小：约 350MB（压缩包）
- **服务端程序**：`RobotArmServer.exe`（含在 `RobotArmServer.zip` 中）
  下载地址：[FlexArm v2.0.0 Release](https://github.com/hamlet0168/flexarm/releases/tag/v2.0.0)  
  最新版本：v2.0.0
  文件大小：约 231 MB（压缩包）

- **安装步骤**:
  1. 从上述地址下载 `RobotArmServer.zip`
...
| # | 方法 | 接口 | 说明 |
|---|------|------|------|
| 14 | POST | `/api/go_home` | Home 回到桌面 |
| 15 | POST | `/api/go_back` | Back 返回 |
| 16 | POST | `/api/go_forward` | Forward 前进 |
| 17 | POST | `/api/reset` | 机械臂复位 |
| 18 | POST | `/api/clear_overlay` | 清除画面覆盖框 |
| 19 | GET/POST | `/api/run_app` | 启动指定 APP |
| 20 | POST | `/api/swipe_up` | 上滑(大幅/小幅) |
| 21 | POST | `/api/swipe_down` | 下滑(大幅/小幅) |
| 22 | POST | `/api/swipe_up_normal` | 标准上划(成功率 ~80%) |
| 23 | POST | `/api/swipe_down_normal` | 标准下划 |
| 24 | POST | `/api/swipe` | 自定义滑动(指定起点/终点百分比) |
| 25 | POST | `/api/close_all_apps` | 关闭所有后台 APP |
| 26 | POST | `/api/click_icon` | 模板匹配点击图标 |
| 27 | POST | `/api/click_icons` | 依次点击多个图标 |
| 28 | POST | `/api/click_icon_many_times` | 同位置连点多次 |
| 29 | POST | `/api/click_text` | OCR 找文字并点击 |
| 30 | POST | `/api/click_at` | 画面像素坐标点击 |
| 31 | POST | `/api/click` | 手机百分比坐标点击 |
| 32 | POST | `/api/click_roi` | ROI 区域中心点击 |
| 33 | POST | `/api/screenshot` | 截图(保存文件 / 返回 base64) |
| 34 | POST | `/api/reload_gestures` | 重新加载手势配置 |

### 视觉检测(12 个)

| # | 方法 | 接口 | 说明 |
|---|------|------|------|
| 35 | POST | `/api/find_template` | 全屏模板匹配找图标 |
| 36 | POST | `/api/find_template_roi` | ROI 区域内模板匹配 |
| 37 | POST | `/api/find_text` | OCR 找文字 |
| 38 | POST | `/api/find_text_roi` | ROI 区域内 OCR 找文字 |
| 39 | POST | `/api/find_all_text` | 识别画面所有文字 |
| 40 | POST | `/api/find_all_templates` | 所有模板都必须匹配 |
| 41 | POST | `/api/find_any_template` | 任一模板匹配即可 |
| 42 | POST | `/api/count_template` | 统计模板出现次数 |
| 43 | POST | `/api/detect_desktop` | 检测当前桌面页面 |
| 44 | POST | `/api/detect_page` | 检测当前页面 |
| 45 | POST | `/api/wait_for_template` | 轮询等待模板出现 |
| 46 | POST | `/api/wait_for_page` | 轮询等待指定页面 |

### 脚本控制(4 个)

| # | 方法 | 接口 | 说明 |
|---|------|------|------|
| 47 | POST | `/api/run_script` | 执行 YAML 脚本(异步启动) |
| 48 | GET | `/api/script_status` | 查询脚本是否运行中 |
| 49 | GET | `/api/script_progress` | 查询脚本执行进度 |
| 50 | POST | `/api/stop_script` | 强制中断脚本 |

### 配置管理(6 个)

| # | 方法 | 接口 | 说明 |
|---|------|------|------|
| 51 | GET/PUT | `/api/config/daily` | 读取/更新每日自动化配置 |
| 52 | GET/PUT | `/api/config/app/<name>` | 读取/更新 APP 页面配置 |
| 53 | GET/PUT | `/api/config/gesture` | 读取/更新手势配置 |

### 系统管理(1 个)

| # | 方法 | 接口 | 说明 |
|---|------|------|------|
| 54 | POST | `/api/shutdown` | 优雅关闭服务 |
